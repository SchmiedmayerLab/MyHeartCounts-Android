...
class MHCStudyBundleTest {
    ...
    private val bundleDir: File = File(GENERATED_ROOT)
        .walkTopDown()
        .firstOrNull { it.isDirectory && it.name == BuildConfig.STUDY_BUNDLE_ASSET_PATH }
        ?: error("No study bundle under $GENERATED_ROOT — run ./gradlew :myheartcounts:assembleDebug")

    private val bundle: StudyBundle = StudyBundle.open(bundleDir)
    ...
    fun `resolves the consent document the app falls back to`() {
        // given
        val fallback = bundleDir.resolve(FALLBACK_CONSENT_PATH)
    ...
    private companion object {
        /**
         * Root the build unpacks the pinned bundle beneath, relative to the module directory.
         */
        const val GENERATED_ROOT = "build/generated"
        const val FALLBACK_CONSENT_PATH = "consent/Consent+en-US.md"
